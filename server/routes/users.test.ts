...
describe('PATCH /api/v1/users', () => {
  ...
  it('should return 201 when creating a new profile', async () => {
    const fakeProfile: UpdateUsersData = {
      username: 'apple',
      pronouns: 'she/her',
      bio: 'She is an apple',
    }

    vi.mocked(db.updateProfile).mockResolvedValue()
    const response = await request(server)
      .patch('/api/v1/users')
      .set('authorization', `Bearer ${getMockToken()}`)
      .send(fakeProfile)
  ...
  it('should return 500 when no access token is passed', async () => {
    const fakeProfile: UpdateUsersData = {
      username: 'apple',
      pronouns: 'she/her',
      bio: 'She is an apple111',
    }

    vi.mocked(db.updateProfile).mockRejectedValue(new Error('test'))
    const response = await request(server)
      .patch('/api/v1/users')
      .set('authorization', `Bearer ${getMockToken()}`)
      .send(fakeProfile)
